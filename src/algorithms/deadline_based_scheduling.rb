  ...
  class DeadlineBasedScheduling
    ...
    def schedule_job(job_list, worker_status, arg={})
      ...
      job_id_by_priority.each{ |job_id|
        ...
        job.avg_running_time = avg_time if avg_time != nil && avg_time > 0
    ...
    def get_required_worker_range(job_id, job, worker_by_throughput, current_timestamp, job_running_time, worker_avg_running_time)
      ...
      if current_timestamp > job.deadline
        @logger.warn "#{job_id} is going missed deadline; give as much as possble"
      ...
      required_throughput = job.task.size / Float(job.deadline-current_timestamp)
      worker_by_throughput.each_with_index do |worker_id|
        ...
        if job.avg_task_running_time != nil && job.avg_task_running_time > 0 
          @logger.info "Estimate run time using average task execution"
          estimated_running_time = job.avg_task_running_time
        elsif job.task_running_time_on_worker[worker_id] != nil
          @logger.info "Estimate run time using provided runtime"
          ...
          if avg != nil && avg > 0
            @logger.info "Estimate run time using system average"
            estimated_running_time = avg
          else
            @logger.info "Estimate run time using default value"
