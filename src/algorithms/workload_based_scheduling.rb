  ...
  class WorkloadBasedScheduling
    ...
    def schedule_job(job_list, worker_status, arg={})
      ...
      workload_list = Hash.new{ |hash, key|
        hash[key] = 1.0 * job.progress.undone.size * job.task_running_time_on_worker.values.reduce(:+) / job.task_running_time_on_worker.size
      }
      ...
      job_id_by_workload.each{ |job_id|
        ...
        additional_worker_count = [expected_worker_count[job_id].floor, job_list[job_id].progress.undone.size - job_worker_count[job_id], remaining_worker_count].min
      ...
      job_id_by_workload.each{ |job_id|
        break if remaining_worker_count == 0
        additional_worker_count = [job_list[job_id].progress.undone.size - job_worker_count[job_id], remaining_worker_count].min
        remaining_worker_count -= additional_worker_count
        job_worker_count[job_id] += additional_worker_count
      }
