...
def get_job(deadline=Time.now+300)
  j = Job.new
  5.times { j.add_task SleepTask.new(20)}
  j.deadline = deadline
  j.priority = rand(1...20)
  return j
end
def get_gpu_job(deadline=Time.now+300)
  j = Job.new
  5.times { j.add_task GPUSleepTask.new(20)}
  j.deadline = deadline
  j.priority = rand(1...20)
  return j
end
...
c.submit_jobs([get_job, get_job, get_job])
c.submit_jobs([get_job, get_job, get_job])
c.submit_jobs([get_gpu_job, get_gpu_job, get_gpu_job])
c.wait_all
